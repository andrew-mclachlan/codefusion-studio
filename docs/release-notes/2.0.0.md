...
- **[Embedded AI Tools](../user-guide/tools/manage-ai-models.md)**: Available in System Planner when using supported processors. This tool lets you import supported model files, assign them to cores, configure settings, run compatibility and profiling checks, and generate source code for deployment.
...
- **[Memory Allocation enhancements](../user-guide/tools/memory-allocation.md)**: Includes an interactive memory bar that lets you create or edit partitions directly from the visual layout. Clicking on an available region prepopulates the form to speed up configuration.
- **[Peripheral Allocation enhancements](../user-guide/tools/peripheral-allocation.md)**: Provides a more intuitive setup flow—from the moment you allocate a peripheral through to pin assignment and final configuration. Deletion and navigation across core projects are also easier, with visual cues throughout the entire setup flow to guide your actions.
- **[Profiling](../user-guide/tools/profiling.md)**: Adds support for Zephelin profiling in Zephyr-based projects. This feature enables developers to capture runtime and AI inference-level performance data, and prepare captured CTF traces for visualization in the Zephelin Trace Viewer. Profiling can be enabled and configured directly from the System Planner for supported processors.
...
- Visual C++ Redistributable requirement on Windows  
    - The AI Tools (including the `cfsai` utility and Embedded AI Tools) on Windows require the [:octicons-link-external-24: Visual C++ Redistributable for Visual Studio 2015](https://aka.ms/vs/16/release/vc_redist.x64.exe){:target="_blank"} to be installed. Without this redistributable, the tools may fail to run correctly.
